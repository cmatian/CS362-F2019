...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case minion:
        if (choice1 || choice2) // If either are true (1)
        {
            //+1 action
            state->numActions++;
        ...
        {
            printf("You must set choice1 or choice2 to 1 in order to play this card.\n");
            return -1;
        }
    ...
    case tribute:
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            else if (tributeRevealedCards[i] == -1 || tributeRevealedCards[i] == curse)
            {
                // Skip duplicate entries or curse cards
                continue;
            }
            else
            { //Action Card
