...
int scoreFor (int player, struct gameState *state) {
    ...
    //score from deck
    for (i = 0; i < state->deckCount[player]; i++)  //bug fixed
    {
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
        ...
        if ( (getCost(state->hand[currentPlayer][choice1]) + 3) < getCost(choice2) )    //bug fixed
        {
            return -1;
        }
    ...
    case remodel:
        ...
        if ( (getCost(state->hand[currentPlayer][choice1]) + 2) < getCost(choice2) )    //bug fixed
        {
            return -1;
        }
